...
function CartDisplay({state,dispatch}) {
    ...
    return (
        <div className='CartDis p-5 rounded-md block bg-white h-fit min-w-72'>
                        ...
                        <div className='border-b flex justify-between p-1' key={i}>
                            <div className='block text-xs'>
                                ...
                                <div className='flex gap-2'>
                                    ...
                                    <span className='text-[hsl(14_25%_72%)] font-medium'>@ ${item.unit_price}</span>
                                    <span className='text-[hsl(7_20%_60%)] font-semibold'>${item.total_item_price}</span>
                                </div>
                            </div>
            ...
            <div className='flex justify-between h-12'><h1 className='text-xs font-semibold flex items-center'>Order Total</h1><span className='text-lg font-bold flex items-center'>${state.total_price}</span></div>
            <div className='mb-4 w-full h-12 text-xs bg-[hsl(20_50%_98%)] flex items-center justify-center rounded-md'> 
                ...
                <span className='flex justify-center w-full'><button className='bg-[hsl(14_86%_42%)] w-[90%] h-10 rounded-full text-xs text-white font-semibold hover:bg-[hsl(14_86%_37%)] transition-all'>Confirm Order</button></span>
            </div>}
            {state.total_ni===0&&
